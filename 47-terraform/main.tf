...
resource "aws_key_pair" "Key-47" {
  key_name   = "Key-47"
  public_key = tls_private_key.ec2-key.public_key_openssh # Path to your public key file
  
}

resource "aws_security_group" "ec2-sg-47" {
  name        = "ec2-sg-47"
  description = "Security group to access EC2 instance - 47 pamoka"
...
resource "aws_instance" "ubuntu-47" {
  ami           = "ami-020cba7c55df1f615" # Example AMI ID, replace with a valid one
  instance_type = "t2.micro"
  key_name = aws_key_pair.Key-47.key_name
  security_groups = [aws_security_group.ec2-sg-47.name]
